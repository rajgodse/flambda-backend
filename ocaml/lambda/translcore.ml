                ...
                {exp_desc =
                   Texp_function
                     { arg_label = _; param = param'; cases = cases';
                       partial = partial'; region = region'; curry = curry';
                       warnings = warnings'; arg_mode = arg_mode'; arg_sort;
                       ret_sort };
      ...
      event_function_rhs ~scopes case.c_rhs
        (function repr ->
           transl_curried_function ~scopes ~arg_sort:param_sort ~arg_layout
             ~arg_mode:(Amode Global) ~return_sort:case_sort ~return_layout
             rhs_loc repr ~region:true ~curry partial warnings param [case])
    in
    let attr = default_function_attribute in
